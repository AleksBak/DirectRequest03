...
namespace DirectRequest
{
	...
	public partial class BasicProgressBar : Control
	{
		...
		private Color positiveValuesBarColor = Color.Red;
		private Color negativeValuesBarColor = Color.DodgerBlue;
		...
		public BasicProgressBar()
		{
			...
			this.ForeColor = positiveValuesBarColor;
		...
		{
			positiveValuesBarColor = ForeColor;
		...
		/// <summary>
		/// The positive values progress bar color.
		/// </summary>
		[Description("The positive values progress bar color."),
		DefaultValue(typeof(Color), "Red")]
		public Color PositiveValuesColor
		{
			get { return positiveValuesBarColor; }
			set
			{
				positiveValuesBarColor = value;
				this.Invalidate();
			}
		}

		/// <summary>
		/// The negative values progress bar color.
		/// </summary>
		[Description("The negative values progress bar color."),
		DefaultValue(typeof(Color), "Blue")]
		public Color NegativeValuesColor
		{
			get { return negativeValuesBarColor; }
			set
			{
				negativeValuesBarColor = value;
				this.Invalidate();
			}
		}
		...
		protected override void OnPaint(PaintEventArgs pe)
		{
			...
			try
			{
				...
				if (currentValue != 0)
				{
					using (SolidBrush positiveValuesBrush = new SolidBrush(positiveValuesBarColor), negativeValuesBrush = new SolidBrush(negativeValuesBarColor))
					{
						if (orientation == Orientation.Vertical)
						{
							//int scaledHeight = this.Height - Convert.ToInt32(((double)this.Height / maximum) * currentValue);
							//int scaledHeight = this.Height - Convert.ToInt32(((double)this.Height / (Math.Abs(maximum) - Math.Abs(minimum))) * currentValue);
							int scaledHeight = this.Height - Convert.ToInt32(((double)this.Height / (maximum - minimum)) * (currentValue - minimum));
							pe.Graphics.FillRectangle(positiveValuesBrush, 0, scaledHeight, this.Width, this.Height);
						}
						else
						{
							// находим положение нуля
							int scaledZero = Convert.ToInt32(((double)this.Width / (maximum - minimum)) * Math.Abs(minimum));
							int scaledWidth = Convert.ToInt32(((double)this.Width / (maximum - minimum)) * (currentValue - minimum));
							if (currentValue >= 0)
							{
								pe.Graphics.FillRectangle(positiveValuesBrush, scaledZero, 0, scaledWidth - scaledZero, this.Height);
							}
							else
							{
								pe.Graphics.FillRectangle(negativeValuesBrush, scaledWidth, 0, scaledZero - scaledWidth, this.Height);
							}
							// также рисуем нулевую линию если имеются отриц. значения:
							if (this.minimum < 0)
							{
								pe.Graphics.DrawLine(new Pen(Color.Black), scaledZero, 0, scaledZero, this.Height);
							}
						}
					}
				}
